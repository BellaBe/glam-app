...
async def detailed_health_check(lifecycle: LifecycleDep):
    ...
    try:
        ...
        # Check messaging
        if not lifecycle.messaging_wrapper or not lifecycle.messaging_wrapper.is_connected():
            health_status["components"]["messaging"] = "unhealthy"
            health_status["status"] = "degraded"
        
        # Get some basic metrics
        if lifecycle.credit_account_repo:
            zero_balance_merchants = await lifecycle.credit_account_repo.get_merchants_with_zero_balance()
            health_status["metrics"]["zero_balance_accounts"] = len(zero_balance_merchants)
