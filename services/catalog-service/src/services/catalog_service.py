...
class CatalogService:
    ...
    async def get_sync_progress(self, sync_id: str, correlation_id: str) -> SyncProgressOut:
        ...
        if not sync:
            raise NotFoundError(
                message=f"Sync operation {sync_id} not found", resource="sync_operation", resource_id=sync_id
            )
    ...
    async def process_product_batch(
        self, sync_id: str, merchant_id: str, products: list[dict], batch_num: int, has_more: bool, correlation_id: str
    ) -> list[CatalogItemOut]:
    ...
    async def _cache_progress(
        ...
        await self.redis.setex(f"sync:{sync_id}", self.config.get("sync_progress_ttl", 3600), json.dumps(data))
