...
class SchedulerEventPublisher(DomainEventPublisher):
    ...
    async def publish_schedule_created(
        ...
        context = EventContext(
            event_id=str(UUID()),
            subject=SchedulerEvents.SCHEDULE_CREATED,
    ...
    async def publish_schedule_updated(
        ...
        context = EventContext(
            event_id=str(UUID()),
            subject=SchedulerEvents.SCHEDULE_UPDATED,
            correlation_id=correlation_id,
            timestamp=datetime.now(timezone.utc),
            source_service=self.service_name_override,
            metadata={
    ...
    async def publish_execution_started(
        ...
        context = EventContext(
            event_id=str(UUID()),
            subject=SchedulerEvents.EXECUTION_STARTED,
            correlation_id=correlation_id,
            timestamp=datetime.now(timezone.utc),
            source_service=self.service_name_override,
            metadata={
    ...
    async def publish_execution_completed(
        ...
        context = EventContext(
            event_id=str(UUID()),
            subject=SchedulerEvents.EXECUTION_COMPLETED,
            correlation_id=correlation_id,
            timestamp=datetime.now(timezone.utc),
            source_service=self.service_name_override,
            metadata={
        ...
        self.logger.info(
            f"Publishing {context.subject}",
            extra={**context.to_dict(), "status": status, "duration_ms": duration_ms},
        )
    ...
    async def publish_execution_failed(
        ...
        context = EventContext(
            event_id=str(UUID()),
            subject=SchedulerEvents.EXECUTION_FAILED,
            correlation_id=correlation_id,
            timestamp=datetime.now(timezone.utc),
            source_service=self.service_name_override,
            metadata={
