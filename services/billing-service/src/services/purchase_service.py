...
from ..schemas.billing import (
    PurchaseCreateIn, PurchaseOut, PurchaseCreatedOut,
    CreditsPurchasedPayload, Platform
)
from ..exceptions import (
    MerchantNotFoundError, InvalidCreditPackError,
    PurchaseNotFoundError, PlatformCheckoutError
)
...
class PurchaseService:
    ...
    async def create_purchase(
        ...
        if data.platform == Platform.SHOPIFY:
            try:
                ...
                await self.purchase_repo.update_platform_charge_id(
                    UUID(purchase.id),
                    charge_id
                )
                ...
                raise PlatformCheckoutError(data.platform.value, str(e))
        else:
            # Other platforms not implemented yet
            await self.purchase_repo.fail_purchase(UUID(purchase.id))
            raise PlatformCheckoutError(
                data.platform.value,
    ...
    async def handle_purchase_webhook(
        self,
        charge_id: str,
        status: str,
        merchant_id: UUID
        ...
        if status == "accepted" or status == "active":
            ...
            await self.billing_repo.update_purchase_totals(
                UUID(purchase.merchant_id),
                purchase.credits
            )
