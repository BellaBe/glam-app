...
class BillingRepository:
    ...
    async def find_by_merchant_id(self, merchant_id: UUID) -> Optional[BillingRecord]:
        """Find billing record by merchant ID"""
        return await self.prisma.billingrecord.find_unique(
    ...
    async def update_purchase_totals(
        self,
        merchant_id: UUID,
        credits: int
    ) -> BillingRecord:
        ...
        return await self.prisma.billingrecord.update(
            where={"merchant_id": str(merchant_id)},
            data={
                "total_credits_purchased": {"increment": credits},
