...
class BillingAccount(Base):
    ...
    merchant_id: Mapped[str] = mapped_column(UUID(as_uuid=False), unique=True, index=True, nullable=False)
...
class Payment(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=False), primary_key=True, default=lambda: str(uuid4()))
    merchant_id: Mapped[str] = mapped_column(UUID(as_uuid=False), index=True, nullable=False)
    ...
    status: Mapped[PaymentStatus] = mapped_column(String, nullable=False, default=PaymentStatus.PENDING)
    platform_name: Mapped[str] = mapped_column(String, nullable=False)
    platform_charge_id: Mapped[str | None] = mapped_column(String, unique=True, index=True, default=None)
