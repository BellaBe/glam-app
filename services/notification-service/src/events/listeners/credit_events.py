...
class CreditEventsListener(Listener):
    ...
    def subject(self) -> str:
        return "evt.credits.>"  # Note: credits not credit

    @property
    def queue_group(self) -> str:
        return "notification-credit-events"
    ...
    async def on_message(self, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            if event_type == Subjects.CREDIT_TRIAL_EXHAUSTED:
                await self._handle_trial_exhausted(event_data, data.get("correlation_id"))
            elif event_type == Subjects.CREDIT_BALANCE_LOW:
                await self._handle_low_balance(event_data, data.get("correlation_id"))
            elif event_type == Subjects.CREDIT_BALANCE_DEPLETED:
                await self._handle_exhausted(event_data, data.get("correlation_id"))
            else:
    ...
    async def _handle_trial_exhausted(self, event_data: dict, correlation_id: str):
        """Trial credits exhausted - upgrade prompt"""
        event_data["_template_override"] = "trial_exhausted"
        event_data["_priority"] = "high"

        notification = await self.notification_service.process_event(
            event_type="evt.credits.trial.exhausted.v1",
    ...
    async def _handle_low_balance(self, event_data: dict, correlation_id: str):
        """Low balance warning - <100 credits"""
        event_data["_template_override"] = "credit_warning"
        event_data["_priority"] = "medium"

        notification = await self.notification_service.process_event(
            event_type="evt.credits.low_balance.v1", event_data=event_data, correlation_id=correlation_id or "unknown"
        )

        await self._publish_result(notification)

    async def _handle_exhausted(self, event_data: dict, correlation_id: str):
        """Credits fully exhausted - service interruption warning"""
        event_data["_template_override"] = "zero_balance"
        event_data["_priority"] = "high"

        notification = await self.notification_service.process_event(
            event_type="evt.credits.exhausted.v1", event_data=event_data, correlation_id=correlation_id or "unknown"
        )
