...
class SelfieService:
    ...
    async def create_analysis(
        ...
        if not customer_id and not anonymous_id:
            raise ValidationError(
                message="Either customer_id or anonymous_id is required",
    ...
    async def get_analysis_status(self, analysis_id: str, merchant_id: str) -> Dict[str, Any]:
        ...
        if analysis.status == AnalysisStatus.PROCESSING:
            ...
            elapsed = (datetime.now(timezone.utc) - analysis.created_at).seconds
            if elapsed < 5:
                progress = 20
    ...
    async def claim_analyses(
        ...
        if not customer_id or not anonymous_id:
            raise ValidationError(
                message="Both customer_id and anonymous_id required for claim",
                code="VALIDATION_ERROR"
            )
    ...
    async def process_ai_analysis(
        ...
        try:
            ...
            self.logger.error(
                f"AI analysis failed for {analysis_id}: {e}",
