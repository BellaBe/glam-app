...
class AnalysisRepository:
    ...
    async def create(self, dto: AnalysisCreate) -> AnalysisOut:
        analysis = await self.prisma.analysis.create(
            data=dto.model_dump()
        )
        return AnalysisOut.model_validate(analysis)
    
    async def find_by_id(self, analysis_id: str) -> Optional[AnalysisOut]:
        analysis = await self.prisma.analysis.find_unique(
            where={"id": analysis_id}
        )
        return AnalysisOut.model_validate(analysis) if analysis else None
    
    async def find_by_hash(self, merchant_id: str, image_hash: str) -> Optional[AnalysisOut]:
        analysis = await self.prisma.analysis.find_unique(
            where={
                "merchant_id_image_hash": {
