...
from fastapi import APIRouter, File, Form, UploadFile, BackgroundTasks, Header, Response, status
from fastapi.responses import JSONResponse
from shared.api import ApiResponse, success_response
from shared.api.dependencies import RequestContextDep, PlatformContextDep
...
@router.post(
    ...
    summary="Create selfie analysis"
)
async def create_analysis(
    ...
    # Add links
    response.links = {
        "self": f"/api/v1/analyses/{analysis.id}",
        "status": f"/api/v1/analyses/{analysis.id}/status"
    }
...
async def get_analysis_status(
    ...
    validate_shop_context(
        ...
        platform_ctx=platform,
        logger=logger,
        expected_scope="selfie:read"
    )
    
    # Get status
    status_data = await svc.get_analysis_status(
...
@router.post(
    ...
    summary="Claim anonymous analyses"
)
