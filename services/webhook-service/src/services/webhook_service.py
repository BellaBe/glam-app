...
from ..repositories.platform_configuration_repository import (
...
class WebhookService:
    ...
    async def process_webhook(
        ...
        async with get_db_session() as session:
            ...
            try:
                ...
                domain_event = self._transform_to_domain_event(
                    platform, topic, payload, shop_id
                )
                if domain_event:
    ...
    def _generate_webhook_id(
        self, platform: str, topic: str, shop_id: str, payload: Dict[str, Any]
    ) -> str:
    ...
    async def _store_for_deduplication(self, webhook_id: str) -> None:
        ...
        try:
            ...
            await self.redis_client.setex(
                f"webhook:processed:{webhook_id}", ttl_seconds, "1"
            )
        except Exception as e:
            self.logger.warning(f"Redis deduplication storage failed: {e}")

    def _transform_to_domain_event(
        self, platform: str, topic: str, payload: Dict[str, Any], shop_id: str
    ) -> Optional[Any]:
