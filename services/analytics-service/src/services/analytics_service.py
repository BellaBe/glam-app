...
class AnalyticsService:
    ...
    async def record_shopper_analysis(
        self,
        payload: SelfieAnalysisCompletedPayload
    ) -> None:
    ...
    async def record_match_metrics(
        self,
        payload: RecommendationMatchCompletedPayload
    ) -> None:
    ...
    async def get_summary_analytics(
        ...
        try:
            # Get today's metrics
            today_metrics = await self.metrics_repo.get_daily_metrics(
                merchant_id=merchant_id,
                date=today
            )
        except Exception as e:
        ...
        # Get catalog metrics
        catalog_metrics = await self.metrics_repo.get_catalog_metrics(
            merchant_id=merchant_id
        )
        
        # Get performance metrics
        performance_metrics = await self.metrics_repo.get_performance_metrics(
            merchant_id=merchant_id,
            date=today
        )
    ...
    async def get_shopper_analytics(
        ...
        if shopper_data.overview.total_shoppers < 10:
            self.logger.warning(
                ...
                extra={
                    "merchant_id": str(merchant_id),
                    "total_shoppers": shopper_data.overview.total_shoppers
                }
            )
    ...
    async def get_season_distribution(
        ...
        if not season_data.get("distribution"):
            raise InsufficientDataError(
                metric_type="season distribution",
                minimum_required=1,
                actual_count=0
            )
    ...
    async def get_product_performance(
        ...
        if limit < 1 or limit > 100:
            raise InvalidPeriodError(
                period=f"limit={limit}",
                valid_periods=["1-100"]
            )
        
        try:
            product_data = await self.metrics_repo.get_product_performance(
                merchant_id=merchant_id,
                limit=limit
            )
        except Exception as e:
            raise MetricsNotFoundError(
                merchant_id=str(merchant_id),
                metric_type="product performance"
            )
    ...
    async def get_credit_analytics(
        ...
        try:
            credit_data = await self.metrics_repo.get_credit_analytics(
                merchant_id=merchant_id
            )
        except Exception as e:
            raise MetricsNotFoundError(
                merchant_id=str(merchant_id),
                metric_type="credit usage"
            )
        
        self.logger.info(
            "Generated credit analytics",
            extra={
                "correlation_id": correlation_id,
                "merchant_id": str(merchant_id)
            }
    ...
    async def _calculate_trends(
        ...
        try:
            ...
            if last_week_metrics.unique_shoppers > 0:
                growth = ((this_week_metrics.unique_shoppers - last_week_metrics.unique_shoppers) 
                         / last_week_metrics.unique_shoppers * 100)
            ...
            match_rate = (
                ...
                if this_week_metrics.total_shoppers > 0 else 0.0
            )
