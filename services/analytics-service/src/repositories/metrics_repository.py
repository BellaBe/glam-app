...
class MetricsRepository:
    ...
    async def get_daily_metrics(self, merchant_id: UUID, date: date) -> TodayMetrics:
    ...
    async def get_aggregated_metrics(self, merchant_id: UUID, start_date: date, end_date: date) -> Last30DaysMetrics:
        ...
        if not daily_metrics:
            raise MetricsNotFoundError(
                merchant_id=str(merchant_id), metric_type="daily", period=f"{start_date} to {end_date}"
            )
    ...
    async def update_season_distribution(
        ...
        await self.prisma.seasondistribution.upsert(
            ...
            update={
                ...
                "avg_confidence": {"set": await self._calculate_running_avg(merchant_id, date, season, confidence)},
            },
        )
    ...
    async def update_credit_usage_metrics(
        ...
        await self.prisma.creditusagemetrics.upsert(
            ...
            update={"credits_consumed": {"increment": credits_consumed}},
        )

    async def update_daily_credit_balance(self, merchant_id: UUID, date: date, credits_remaining: int) -> None:
        """Update daily credit balance"""
        await self.prisma.dailymerchantmetrics.update_many(
            where={"merchant_id": str(merchant_id), "date": datetime.combine(date, datetime.min.time())},
            data={"credits_remaining": credits_remaining},
        )

    async def update_catalog_metrics(
        self, merchant_id: UUID, date: date, total_products: int, analyzed_products: int
    ) -> None:
    ...
    async def get_catalog_metrics(self, merchant_id: UUID) -> dict:
        ...
        latest = await self.prisma.dailymerchantmetrics.find_first(
            where={"merchant_id": str(merchant_id)}, order_by={"date": "desc"}
        )

        if not latest:
            return {"total_products": 0, "analyzed_products": 0, "analysis_coverage": 0.0, "products_never_matched": 0}

        # Count products never matched
        never_matched = await self.prisma.productmatchmetrics.count(
            where={"merchant_id": str(merchant_id), "total_matches": 0}
        )
    ...
    async def get_performance_metrics(self, merchant_id: UUID, date: date) -> dict:
        ...
        if not metrics:
            return {"analysis_success_rate": 100.0, "avg_processing_time": 0, "failed_analyses_today": 0}
    ...
    async def get_shopper_metrics(self, merchant_id: UUID, start_date: date, end_date: date) -> ShopperAnalyticsData:
        ...
        if not daily_metrics:
            ...
            return ShopperAnalyticsData(
                overview=ShopperOverview(total_shoppers=0, unique_shoppers=0, returning_shoppers=0, return_rate=0.0),
                engagement=ShopperEngagement(analyses_per_shopper=0.0, match_rate=0.0, avg_confidence=0.0),
                daily_breakdown=[],
            )
        ...
        daily_breakdown = [
            DailyShopperBreakdown(date=m.date.strftime("%Y-%m-%d"), shoppers=m.total_analyses, matches=m.total_matches)
    ...
    async def get_season_distribution(self, merchant_id: UUID, start_date: date, end_date: date) -> dict:
        ...
        for dist in distributions:
            if dist.season not in season_totals:
                season_totals[dist.season] = {"shopper_count": 0, "confidence_sum": 0, "product_coverage": 0}
        ...
        for season, data in season_totals.items():
            ...
            product_coverage = await self.prisma.productmatchmetrics.count(
                where={"merchant_id": str(merchant_id), "primary_seasons": {"has": season}}
            )
        ...
        underserved = [d["season"] for d in distribution if d["product_coverage"] < total_products * 0.05]
    ...
    async def get_product_performance(self, merchant_id: UUID, limit: int) -> dict:
        """Get product performance data"""
        # Get top products
        top_products = await self.prisma.productmatchmetrics.find_many(
            where={"merchant_id": str(merchant_id)}, order_by={"total_matches": "desc"}, take=limit
        )

        # Get dead stock (never matched)
        dead_stock = await self.prisma.productmatchmetrics.find_many(
            where={"merchant_id": str(merchant_id), "total_matches": 0}, take=20
        )

        # Get products by season
        all_products = await self.prisma.productmatchmetrics.find_many(where={"merchant_id": str(merchant_id)})
        ...
        for product in all_products:
            for season in product.primary_seasons or []:
                if season not in season_stats:
                    season_stats[season] = {"count": 0, "score_sum": 0}
    ...
    async def get_credit_analytics(self, merchant_id: UUID) -> dict:
        ...
        today_usage = await self.prisma.creditusagemetrics.aggregate(
            where={"merchant_id": str(merchant_id), "date": datetime.combine(today, datetime.min.time())},
        ...
        last_7_days = await self.prisma.creditusagemetrics.aggregate(
            where={
                "merchant_id": str(merchant_id),
                "date": {"gte": datetime.combine(today - timedelta(days=7), datetime.min.time())},
            },
            _sum={"credits_consumed": True},
        )
        ...
        last_30_days = await self.prisma.creditusagemetrics.aggregate(
            where={
                "merchant_id": str(merchant_id),
                "date": {"gte": datetime.combine(today - timedelta(days=30), datetime.min.time())},
            },
            _sum={"credits_consumed": True},
        )
        ...
        history = [
            {"date": date_str, "consumed": data["consumed"], "granted": data["granted"]}
    ...
    async def _calculate_running_avg(self, merchant_id: UUID, date: date, season: str, new_value: float) -> float:
