...
from prisma.models import AnalyticsEvent, MatchMetrics, ShopperAnalysis
...
class AnalyticsRepository:
    ...
    async def create_shopper_analysis(
        ...
        existing = await self.prisma.shopperanalysis.find_unique(where={"analysis_id": analysis_id})

        if existing:
            raise DataConsistencyError(
                entity="ShopperAnalysis", expected="unique analysis_id", actual=f"duplicate analysis_id: {analysis_id}"
            )
    ...
    async def create_match_metrics(
        ...
        existing = await self.prisma.matchmetrics.find_unique(where={"match_id": match_id})

        if existing:
            raise DataConsistencyError(
                entity="MatchMetrics", expected="unique match_id", actual=f"duplicate match_id: {match_id}"
            )
    ...
    async def get_events_by_merchant(
        ...
        return await self.prisma.analyticsevent.find_many(
            where=where_clause, order_by={"event_timestamp": "desc"}, take=limit
        )

    async def count_analyses_by_merchant(self, merchant_id: UUID, start_date: datetime, end_date: datetime) -> int:
        """Count shopper analyses for a merchant in date range"""
        return await self.prisma.shopperanalysis.count(
            where={"merchant_id": str(merchant_id), "created_at": {"gte": start_date, "lte": end_date}}
        )

    async def count_unique_shoppers(self, merchant_id: UUID, start_date: datetime, end_date: datetime) -> int:
    ...
    async def get_season_distribution(
        self, merchant_id: UUID, start_date: datetime, end_date: datetime
    ) -> dict[str, int]:
        """Get distribution of primary seasons"""
        analyses = await self.prisma.shopperanalysis.find_many(
            where={"merchant_id": str(merchant_id), "created_at": {"gte": start_date, "lte": end_date}},
            select={"primary_season": True},
        )
    ...
    async def cleanup_old_events(self, retention_days: int) -> int:
        ...
        try:
            # Delete old events
            result = await self.prisma.analyticsevent.delete_many(where={"event_timestamp": {"lt": cutoff_date}})
