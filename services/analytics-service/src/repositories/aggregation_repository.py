...
class AggregationRepository:
    ...
    async def aggregate_daily_metrics(self, merchant_id: UUID, target_date: date) -> None:
        ...
        try:
            ...
            analyses = await self.prisma.shopperanalysis.count(
                where={"merchant_id": str(merchant_id), "created_at": {"gte": start_time, "lt": end_time}}
            )
            ...
            # Get unique shoppers
            unique_shoppers = await self._count_unique_shoppers(merchant_id, start_time, end_time)

            # Get returning shoppers (had analysis before target date)
            returning_shoppers = await self._count_returning_shoppers(merchant_id, target_date)
            ...
            credits_consumed = await self.prisma.creditusagemetrics.aggregate(
                where={"merchant_id": str(merchant_id), "date": start_time}, _sum={"credits_consumed": True}
            )
            ...
            await self.prisma.dailymerchantmetrics.upsert(
                where={"merchant_id_date": {"merchant_id": str(merchant_id), "date": start_time}},
                create={
                    "merchant_id": str(merchant_id),
            ...
            raise AggregationError(operation="daily_metrics", reason=str(e), merchant_id=str(merchant_id))
    ...
    async def aggregate_all_merchants_daily(self, target_date: date) -> int:
        ...
        for merchant in merchants:
            try:
                await self.aggregate_daily_metrics(UUID(merchant.merchant_id), target_date)
                count += 1
            except Exception as e:
    ...
    async def _count_unique_shoppers(self, merchant_id: UUID, start_time: datetime, end_time: datetime) -> int:
        """Count unique shoppers in time range"""
        analyses = await self.prisma.shopperanalysis.find_many(
            where={"merchant_id": str(merchant_id), "created_at": {"gte": start_time, "lt": end_time}},
            select={"shopper_id": True, "anonymous_id": True},
        )
    ...
    async def _count_returning_shoppers(self, merchant_id: UUID, target_date: date) -> int:
        ...
        for analysis in target_analyses:
            ...
            where_clause = {
                "merchant_id": str(merchant_id),
                "created_at": {"lt": datetime.combine(target_date, datetime.min.time())},
            }
    ...
    async def _find_peak_hour(self, merchant_id: UUID, target_date: date) -> int | None:
        """Find hour with most activity"""
        peak = await self.prisma.hourlyusagemetrics.find_first(
            where={"merchant_id": str(merchant_id), "date": datetime.combine(target_date, datetime.min.time())},
    ...
    async def _get_platform_info(self, merchant_id: UUID) -> dict:
        ...
        event = await self.prisma.analyticsevent.find_first(
            ...
            select={"platform_name": True, "platform_shop_id": True, "domain": True},
        )
        ...
        return {"platform_name": "unknown", "platform_shop_id": "unknown", "domain": "unknown"}
