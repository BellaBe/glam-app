...
class ServiceLifecycle:
    ...
    def __init__(self, config: ServiceConfig, logger: ServiceLogger):
        ...
        # Components
        self.analytics_repo: AnalyticsRepository | None = None
        self.metrics_repo: MetricsRepository | None = None
        self.aggregation_repo: AggregationRepository | None = None
        self.analytics_service: AnalyticsService | None = None
        self.aggregation_service: AggregationService | None = None

        # AggregationJobs
        self.aggregation_jobs: AggregationJobs | None = None
    ...
    def _init_services(self) -> None:
        ...
        self.analytics_service = AnalyticsService(
            analytics_repo=self.analytics_repo, metrics_repo=self.metrics_repo, logger=self.logger
        )

        self.aggregation_service = AggregationService(
            aggregation_repo=self.aggregation_repo, metrics_repo=self.metrics_repo, logger=self.logger
        )
    ...
    async def _init_listeners(self) -> None:
        ...
        listeners = [
            SelfieAnalysisCompletedListener(
                js_client=self.messaging_client, analytics_service=self.analytics_service, logger=self.logger
            ),
            RecommendationMatchCompletedListener(
                js_client=self.messaging_client, analytics_service=self.analytics_service, logger=self.logger
            ),
    ...
    async def _init_aggregation_jobs(self) -> None:
        ...
        self.aggregation_jobs = AggregationJobs(
            config=self.config, aggregation_service=self.aggregation_service, logger=self.logger
        )
