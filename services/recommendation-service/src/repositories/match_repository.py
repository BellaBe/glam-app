...
from ..schemas.recommendation import RecommendationRequest, MatchOut
...
class MatchRepository:
    ...
    async def find_by_id(self, match_id: str) -> Optional[Match]:
        ...
        match = await self.prisma.match.find_unique(
            where={"id": match_id},
            include={"match_items": True}
        )
        return match
    
    async def find_by_analysis_id(self, analysis_id: str) -> Optional[Match]:
        """Find match by analysis ID"""
        match = await self.prisma.match.find_first(
