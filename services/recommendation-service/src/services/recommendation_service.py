...
from ..schemas.recommendation import MatchItemOut, MatchOut, RecommendationRequest, RecommendationResponse
...
class RecommendationService:
    ...
    async def create_recommendation(
        self, merchant_id: UUID, platform_name: str, domain: str, request: RecommendationRequest, correlation_id: str
        ...
        compatible_items = await self.season_client.get_compatible_items(
            merchant_id=str(merchant_id), seasons=seasons, min_score=self.min_score, correlation_id=correlation_id
        )
        ...
        # Create match items
        if compatible_items:
            await self.repository.create_match_items(match_id=match.id, items=compatible_items)

        self.logger.info(
            "Recommendation created",
            extra={"correlation_id": correlation_id, "match_id": match.id, "total_matches": len(compatible_items)},
        )
        ...
        return RecommendationResponse(match_id=match.id, total_matches=len(compatible_items), matches=matches)
    ...
    async def get_match(self, match_id: str) -> MatchOut:
        ...
        if not match:
            raise NotFoundError(message=f"Match {match_id} not found", resource="match", resource_id=match_id)
