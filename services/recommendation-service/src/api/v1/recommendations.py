...
from ...dependencies import EventPublisherDep, LoggerDep, RecommendationServiceDep
from ...schemas.events import MatchCompletedPayload, MatchFailedPayload
from ...schemas.recommendation import MatchOut, RecommendationRequest, RecommendationResponse
...
async def create_recommendation(
    ...
    validate_shop_context(
        client_auth=auth, platform_ctx=platform, logger=logger, expected_scope="recommendations:write"
    )
    ...
    try:
        ...
        return success_response(data=response, request_id=ctx.request_id, correlation_id=ctx.correlation_id)
...
@router.get("/{match_id}", response_model=ApiResponse[MatchOut], summary="Get match details")
async def get_match(
    match_id: str,
    svc: RecommendationServiceDep = None,
    ctx: RequestContextDep = None,
    ...
    # Validate shop context
    validate_shop_context(client_auth=auth, platform_ctx=platform, logger=logger, expected_scope="recommendations:read")
    ...
    if str(match.merchant_id) != auth.shop:
        raise ValidationError(message="Match does not belong to this merchant", field="match_id", value=match_id)

    return success_response(data=match, request_id=ctx.request_id, correlation_id=ctx.correlation_id)
