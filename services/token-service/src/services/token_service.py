...
class TokenService:
    ...
    def __init__(self, repository: TokenRepository, encryption: EncryptionService, logger: ServiceLogger):
        self.repository = repository
        self.encryption = encryption
        self.logger = logger

    async def store_token(self, request: StoreTokenRequest, correlation_id: str) -> str:
        """Store or update a token"""

        # Validate platform
        if request.platform_name not in SUPPORTED_PLATFORMS:
            raise ValidationError(
                f"Unsupported platform: {request.platform_name}", field="platform_name", value=request.platform_name
            )

        # Validate token type
        if request.token_type not in TOKEN_TYPES:
            raise ValidationError(
                f"Invalid token type: {request.token_type}", field="token_type", value=request.token_type
            )
    ...
    async def delete_token(self, merchant_id: str, platform: str, correlation_id: str) -> bool:
        ...
        if deleted:
            ...
            self.logger.info(
                ...
                extra={"correlation_id": correlation_id, "merchant_id": merchant_id, "platform": platform},
            )
            return True
