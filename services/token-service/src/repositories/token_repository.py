...
class TokenRepository:
    ...
    async def find_by_merchant(self, merchant_id: str) -> list[PlatformToken]:
        ...
        return await self.prisma.platformtoken.find_many(where={"merchant_id": merchant_id})

    async def find_by_merchant_platform(self, merchant_id: str, platform_name: str) -> list[PlatformToken]:
        """Find tokens for merchant and platform"""
        return await self.prisma.platformtoken.find_many(
            where={"merchant_id": merchant_id, "platform_name": platform_name}
        )

    async def update_access(self, token_id: str, accessed_by: str) -> None:
    ...
    async def delete(self, merchant_id: str, platform_name: str) -> PlatformToken | None:
        """Delete a token"""
        try:
            return await self.prisma.platformtoken.delete(
                where={"merchant_id_platform_name": {"merchant_id": merchant_id, "platform_name": platform_name}}
            )
        except Exception:
            return None
