...
class WooCommerceAdapter(PlatformAdapter):
    ...
    async def authenticate(self, credentials: dict[str, Any]) -> dict[str, str]:
        ...
        try:
            ...
            creds = await self.token_client.get_woocommerce_credentials(domain=domain, correlation_id=correlation_id)
    ...
    async def fetch_products(
        ...
        headers = await self.authenticate({"domain": domain, "correlation_id": correlation_id})
                ...
                try:
                    async with session.get(
                        ...
                        if response.status == 401:
                            raise UnauthorizedError(
                                f"Invalid WooCommerce credentials for {domain}", auth_type="woocommerce_api"
                            )
                    ...
                    raise InfrastructureError(
                        f"Failed to fetch products from WooCommerce: {e}", service="woocommerce_api", retryable=True
                    )

        self.logger.info(
            "Completed WooCommerce product fetch",
            extra={
    ...
    async def _fetch_variation(
        self, session: aiohttp.ClientSession, domain: str, product_id: int, variation_id: int, headers: dict[str, str]
    ) -> dict[str, Any]:
