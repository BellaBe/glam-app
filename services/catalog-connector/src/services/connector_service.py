...
class ConnectorService:
    ...
    def __init__(self, event_publisher, logger: ServiceLogger, config: dict):
    ...
    async def process_sync_request(
        ...
        if not adapter:
            ...
            raise NotFoundError(
                f"Platform adapter not found: {platform_name}", resource="platform_adapter", resource_id=platform_name
            )
        ...
        try:
            ...
            async for batch in adapter.fetch_products(
                ...
                await self.event_publisher.platform_products_fetched(batch_data=batch, correlation_id=correlation_id)
            ...
            await self.event_publisher.platform_fetch_completed(
                merchant_id=merchant_id, sync_id=sync_id, total_products=total_products, correlation_id=correlation_id
            )
            ...
            if e.retryable:
                self.logger.warning(
                    f"Retryable error: {e}",
                    extra={"correlation_id": correlation_id, "sync_id": sync_id, "error": str(e)},
                )
                raise  # Let listener handle retry
            else:
                await self.event_publisher.platform_fetch_failed(
                    merchant_id=merchant_id, sync_id=sync_id, error=str(e), correlation_id=correlation_id
                )
                raise
