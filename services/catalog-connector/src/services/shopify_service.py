...
class ShopifyService:
    ...
    async def validate_connection(self, connection: StoreConnection) -> bool:
        ...
        async with ShopifyClient(
            merchant_domain=connection.shopify_domain,
    ...
    async def fetch_products(
        ...
        async with self.rate_limit_service.rate_limit_context(connection.store_id, endpoint):
            async with ShopifyClient(
                merchant_domain=connection.shopify_domain,
                access_token=connection.access_token,
                api_version=connection.api_version,
                ...
                # Fetch products
                response = await client.get_products(limit=limit, page_info=cursor, fields=fields)
                ...
                if response.headers:
                    ...
                    await self.rate_limit_service.update_from_headers(
                        connection.store_id, endpoint, current, limit
                    )
                ...
                return CatalogItemBatch(
                    page=page, items=items, has_more=has_more, cursor=next_cursor
                )
        ...
        async with self.rate_limit_service.rate_limit_context(connection.store_id, endpoint):
            async with ShopifyClient(
                ...
                response = await client.get_products(
                    limit=limit, page_info=cursor, updated_at_min=since
                )
    ...
    async def fetch_all_products(
        ...
        while True:
            batch = await self.fetch_products(
                connection=connection, page=page, cursor=cursor, fields=fields
            )
