...
class BulkOperationService:
    ...
    async def start_fetch_operation(self, sync_request: SyncFetchRequestIn, correlation_id: str) -> FetchOperation:
        ...
        try:
            ...
            async with ShopifyGraphQLClient(self.config, self.logger) as client:
                ...
                task = asyncio.create_task(
                    self._poll_bulk_operation(bulk_op.id, shop_domain, access_token, correlation_id)
                )
                ...
                def _finalize(t: asyncio.Task[None]):
                    ...
                    try:
                        t.result()  # Raise if task failed
                    except Exception as e:
                        self.logger.error(f"Bulk operation task failed: {e}")
    ...
    async def shutdown(self) -> None:
        """Shutdown service and cancel all tasks"""
        self.logger.info("Shutting down BulkOperationService")
        if not self._tasks:
        ...
        self._tasks.clear()
        self.logger.info("BulkOperationService shutdown complete")

    async def _poll_bulk_operation(self, bulk_op_id: UUID, shop_domain: str, access_token: str, correlation_id: str):
        ...
        try:
            ...
            while datetime.utcnow() - start_time < timeout:
                ...
                if status_result.get("completedAt"):
                    bulk_op.completed_at = datetime.fromisoformat(status_result["completedAt"].replace("Z", "+00:00"))
    ...
    async def _handle_bulk_failure(self, bulk_op: BulkOperation, error_message: str | None = None):
    ...
    def _parse_since_timestamp(self, options: dict[str, Any]) -> datetime | None:
