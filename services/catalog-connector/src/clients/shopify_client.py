...
class ShopifyClient(BaseAPIClient):
    ...
    def __init__(
        self, merchant_domain: str, access_token: str, api_version: str = "2024-01", **kwargs
    ):
        # Build base URL
        base_url = f"https://{merchant_domain}/admin/api/{api_version}"
    ...
    async def _request(self, method: str, endpoint: str, **kwargs) -> APIResponse[Dict[str, Any]]:
        ...
        if response.headers:
            ...
            if current >= limit * 0.8:  # 80% threshold
                if self.logger:
                    self.logger.warning(
                        f"Approaching rate limit: {current}/{limit}", extra={"endpoint": endpoint}
                    )
        ...
        if response.status_code == 429:
            ...
            raise RateLimitExceededError(
                store_id=self.base_url.split("/")[2], retry_after=retry_after
            )
        elif response.status_code == 401:
            raise ShopifyAPIError(
                "Invalid access token", status_code=401, response_body=str(response.data)
            )
    ...
    async def get_products(
        ...
        **params,
        ...
        query_params = {"limit": min(limit, 250), **params}  # Shopify max is 250
    ...
    async def get_product(
        self, product_id: str, fields: Optional[List[str]] = None
    ) -> APIResponse[Dict[str, Any]]:
