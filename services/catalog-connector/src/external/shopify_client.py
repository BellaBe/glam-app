...
class ShopifyGraphQLClient:
    ...
    async def __aenter__(self):
        self.client = httpx.AsyncClient(timeout=httpx.Timeout(30.0), limits=httpx.Limits(max_connections=10))
        return self
    ...
    async def execute_query(
        self, shop_domain: str, access_token: str, query: str, variables: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        ...
        async with self._rate_limiter:
            ...
            try:
                ...
                raise ShopifyAPIError(f"HTTP {e.response.status_code}: {e.response.text}") from e
            except Exception as e:
                raise ShopifyAPIError(f"Request failed: {e!s}") from e

    async def start_bulk_operation(self, shop_domain: str, access_token: str, query: str) -> dict[str, Any]:
    ...
    async def get_bulk_operation_status(
        self, shop_domain: str, access_token: str, bulk_operation_id: str
    ) -> dict[str, Any]:
