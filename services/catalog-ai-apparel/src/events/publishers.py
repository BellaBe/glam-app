        ...
        try:
            ...
            if result.status == "success":
                await publisher.publish_event(
                    event_type=CatalogAnalysisEvents.ITEM_ANALYSIS_COMPLETED,
                    payload=result.model_dump(),
                    correlation_id=correlation_id
                )
            else:
                await publisher.publish_event(
                    event_type=CatalogAnalysisEvents.ITEM_ANALYSIS_FAILED,
                    payload=result.model_dump(),
                    correlation_id=correlation_id
                )
                
        except Exception as e:
            logger.error(f"Failed to process catalog item analysis request: {str(e)}", exc_info=True)
