...
class CatalogAnalysisService:
    ...
    async def analyze_catalog_item(self, request: CatalogItemAnalysisRequest) -> CatalogItemAnalysisResult:
        ...
        try:
            ...
            # Build file paths
            variant_dir = Path(self.config.products_base_path) / request.shop_id / request.product_id / request.variant_id
            ...
            self.logger.info(f"Catalog item analysis completed successfully in {latency}ms, found {len(dominant_colours)} colors")
            ...
            latency = int((time.perf_counter() - t0) * 1000)
            self.logger.error(f"Catalog item analysis failed: {str(e)}", exc_info=True)
    ...
    def _get_segmenter(self):
        ...
        from mediapipe.tasks.python.vision.image_segmenter import _RunningMode
    ...
    def _segment_apparel(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray, Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]]:
        """
        Run Mediapipe segmentation and extract apparel regions from catalog item.
        
        Returns:
            coloured_mask: H×W×3 BGR viz of all classes
            apparel_mask: H×W uint8 binary mask (255=apparel, 0=other)
            apparel_crop: H×W×3 BGR image where only apparel pixels survive
            bound_crop: H'×W'×3 BGR tight crop of apparel region
            bound_mask: H'×W' uint8 tight crop of binary mask
        """
        # Get segmenter
        ...
        class_mask = result.category_mask.numpy_view()  # H×W, values 0–5
    ...
    def _extract_apparel_palette_lab(
        ...
        Args:
            mask: H×W uint8 mask (255 = apparel, 0 = other)
