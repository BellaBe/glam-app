...
from src.db.models import STATUS_TRANSITIONS, MerchantStatus
from src.events.publishers import MerchantEventPublisher
from src.exceptions import InvalidStatusTransitionError, MerchantNotFoundError
from src.repositories.merchant_repository import MerchantRepository
from src.schemas.merchant import MerchantOut, MerchantSyncIn, MerchantSyncResponse
...
class MerchantService:
    ...
    async def sync_merchant(
        self, platform_name: str, data: MerchantSyncIn, correlation_id: str
        ...
        async with self.session_factory() as session:
            ...
            existing = await repo.find_by_platform_and_domain(platform_name, data.domain)
            ...
            if existing:
                ...
                self.logger.info(
                    ...
                    extra={"merchant_id": merchant.id, "platform": platform_name},
                )
            ...
            # Publish events
            if created:
                await self.publisher.merchant_created(merchant_out, correlation_id)

            await self.publisher.merchant_synced(merchant_out, first_install=created, correlation_id=correlation_id)

            return MerchantSyncResponse(created=created, merchant_id=UUID(merchant.id))

    async def get_merchant(self, platform_name: str, domain: str) -> MerchantOut:
        """Get merchant by platform and domain"""
        async with self.session_factory() as session:
            repo = MerchantRepository(session)
            merchant = await repo.find_by_platform_and_domain(platform_name, domain)
    ...
    async def handle_app_uninstalled(self, platform_name: str, domain: str, correlation_id: str) -> None:
        ...
        async with self.session_factory() as session:
            ...
            merchant = await repo.update_status(merchant, MerchantStatus.UNINSTALLED)
            await session.commit()

            self.logger.info(
                f"Merchant uninstalled: {domain}",
                extra={"merchant_id": merchant.id, "platform": platform_name, "old_status": old_status},
            )
    ...
    def validate_status_transition(self, current_status: MerchantStatus, new_status: MerchantStatus) -> None:
